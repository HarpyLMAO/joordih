...
export default function command({showres, showcmd, cmd}: props) {
    ...
    const clonedElement = cloneElement(<CommandResponse showres={showres} showcmd={showcmd} cmd={cmd}
                                                        commandInput={commandInput}/>)

    return (
        <div onClick={() => ref.current?.focus()}>
            <form className='flex' onSubmit={handleSubmit}>
                <PATH> root@joordih.lol:~$ </PATH>
                <div className='cursor'>
                    <INPUT type="text" name='commandInput' value={commandInput} onChange={handleChangeAndSize} autoFocus
                           ref={ref} disabled={isDisabled} autoComplete='off'/>
